...
def oldInvoiceLayout() :
    ...
    try :
        sql = "SELECT * FROM bills, customers WHERE bills.contactNo = customers.contactNo AND username = %s"
        val = (username,)
        mycursor.execute(sql, val)
        results = mycursor.fetchall()
        for i in results:
...
welcome = Label(body , text = "Welcome to Cashier Account" , font = ("Arial" , 40))
